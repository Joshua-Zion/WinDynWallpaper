...
| 打包构建 | ✅ 完成 | HashRouter 适配 file 协议，NSIS 安装器 |
...
- [x] **开机自启优化**：支持后台静默启动（已添加 `--hidden` 参数）
...
- 单实例锁：防止应用多开
- 开机自启后台启动：支持 `--hidden` 参数静默启动
- NSIS 安装器：自动关闭旧版进程
- 版本号动态读取：设置页版本从 package.json 获取
...
- WorkerW 获取：独立 GetWorkerW.exe 工具 + 重试机制

**修复：**
- 打包后页面空白：BrowserRouter → HashRouter 适配 file 协议
- 壁纸库/详情页切换后当前壁纸标识消失
